...
void ArmPlugin::Load(physics::ModelPtr _parent, sdf::ElementPtr /*_sdf*/) 
{
	...
	*/
	...
	cameraSub = cameraNode->Subscribe("camera", onCameraMsg(), class_instance);
	...
	*/
	...
	collisionSub = collisionNode->Subscribe("collision", onCollisionMsg(), class_instance);
...
if VELOCITY_CONTROL
	...
	*/
	...
	if (action%2) {velocity += actionVelDelta;}
	else	  {velocity -= actionVelDelta;}
	...
	*/
	...
	if (action%2) {joint += actionJointDelta;}
	else	  {joint -= actionJointDelta;}
		...
		*/ 
		...
		if(!checkGroundContact)
		{
			...
			if( episodeFrames > 1 )
			{
				...
				avgGoalDelta  = 0.0 + avgGoalDelta*alpha + (1-alpha)*distDelta;
				rewardHistory = exp(-avgGoalDelta);
				newReward     = true;	
			}
